####Script to create figure 6 for mortality review
####Heatmap of spatial scale of studies by taxonomic group
####Liam Langley
...
## group site and regional - slightly ambiguous distinction

df_clean$Spatial_scale <- ifelse(df_clean$Spatial_scale == "Site", "Regional", paste(df_clean$Spatial_scale))


#------------------------------------------------#
## Create heatmaps for spatial scale of study ####
#------------------------------------------------#
...
df_ID <- with(df_clean, aggregate(sum, by = list(Paper_ID, Spatial_scale, Taxonomic_group), "sum"))

names(df_ID)[1] <- "Paper_ID"
names(df_ID)[2] <- "Spatial_Scale"
...
df_SSTG <- with(df_ID, aggregate(sum, by = list(Spatial_Scale, Taxonomic_Group), "sum"))

names(df_SSTG)[1] <- "Spatial_Scale"
names(df_SSTG)[2] <- "Taxonomic_Group"
names(df_SSTG)[3] <- "NPapers"

##create heatmap from df_ID

a <- ggplot() + 
      geom_bin2d(data = df_ID, aes(x = Spatial_Scale, y = Taxonomic_Group)) +
      ##add counts to cells
      stat_bin2d(data = df_ID, geom = "text", aes(x = Spatial_Scale, y = Taxonomic_Group, 
                                                  label = ..count..)) +
      ##colour cells using scale_colour_gradient2
      scale_fill_gradient(low = "#FFFF00", high = "#FF3300") +
      labs(x = "Spatial Scale", y = "Taxonomic Group", fill = "No. Papers") +
...
ggsave(plot = a, filename = "figure_6.tiff",
       device = device,
